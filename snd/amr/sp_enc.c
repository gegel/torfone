				   ...
				   int16_t * anap)
{
	...
	memset( cod, 0, sizeof( float )*L_CODE);
	indx = 0;
	rsign = 0;
		    ...
		    int16_t ana[], enum Mode *used_mode, float synth[])
{
	...
	float gain_pit_sf0=0;	/* Quantized pitch gain for sf0 */
	float gain_code_sf0=0;	/* Quantized codebook gain for sf0 */
