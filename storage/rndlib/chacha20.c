...
#ifdef USE_ASM
extern void asm_quarterround(u32* x);
extern void asm_add_and_store(byte* o, u32* x, const u32*input);
#endif
...
void chacha20(unsigned char* blk, unsigned char* out)
{
  ...
  for (i = 0;i < 16;++i) x[i]=U8TO32_LITTLE(blk + 4 * i);

  #ifdef USE_ASM
    asm_quarterround(x);
    asm_add_and_store(output, x, input);
  #else
    for (i = 20; i > 0; i -= 2)
    {
    ...
    for (i = 0;i < 16;++i) x[i] = PLUS(x[i],U8TO32_LITTLE(blk + 4 * i));
    for (i = 0;i < 16;++i) U32TO8_LITTLE(out + 4 * i,x[i]);
  #endif
